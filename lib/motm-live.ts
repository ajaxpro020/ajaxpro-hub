...
export type LiveVoteTrendRow = { bucket:string|Date; votes:number };
...
export const buildLiveVoteTrend = (rows:LiveVoteTrendRow[]) => rows
  .map(row => ({bucket: new Date(row.bucket).toISOString(), votes: Number(row.votes)}))
