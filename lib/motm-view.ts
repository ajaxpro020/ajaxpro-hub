...
export type ClubSection = "home" | "motm" | "stand" | "tools" | "jeugddossiers";
...
const nav = (active: ClubSection, user?: ClubUser) => {
  const hasTools = Boolean(user?.discordRoleIds && (
    ...
    rolesHavePermission(user.discordRoleIds, permissions.jeugddossiersManage)
  ));
  const mainActive=active==="stand"?"motm":active;
  return [["home","/club","Home"],["motm","/club/stand","MOTM"],...(hasTools?[["tools","/club/tools","Tools"]]:[])].map(([key,href,label]) => `<a href="${href}"${mainActive===key ? ' class="active" aria-current="page"' : ""}>${label}</a>`).join("");
};
