...
test("deelmetadata gebruikt de actuele AjaxPro MOTM-kaart",()=>assert.match(shareMetadata(match("open")).image,/\/assets\/motm-share-v2\.jpg$/));
test("openbare share- en niet-gevondenpagina laden stylesheetversie 7",()=>{for(const html of [renderPublicShare(match("open")),renderPublicNotFound()]){assert.match(html,/\/motm\.css\?v=7/);assert.doesNotMatch(html,/\/motm\.css\?v=6/)}});
...
test("share-CTA laat de interne stemroute de bestaande sessie hergebruiken",()=>{const html=renderPublicShare(match("open"));assert.ok(html.includes(`href="${internalVotePath(match("open").slug)}"`));assert.doesNotMatch(html,/href="\/api\/auth\/discord-login/)});
