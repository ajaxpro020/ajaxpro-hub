...
import { esc, errorPage, formatKickoff, formatMoment, matchHeading, page, remainingTime } from "../../lib/motm-view";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if(view==="new") return page("Nieuwe stemming",`<main class="motm-main admin"><a class="back" href="/club/motm/beheer">← Beheer</a><p class="eyebrow">Nieuwe stemming</p><h1>Wedstrijd instellen</h1><p class="sub">Tijden zijn Nederlandse tijd (Europe/Amsterdam). Controleer alles voor je opent.</p><form method="post" action="/club/motm/nieuw"><div class="form-grid"><label>Tegenstander<input required name="opponent" autocomplete="off"></label><label>Competitie<input required name="competition" value="Eredivisie"></label><label>Aftrap<input required type="datetime-local" name="kickoffAt"></label><label>Thuis of uit<select name="homeOrAway"><option value="home">Thuis</option><option value="away">Uit</option></select></label>${scheduleFields()}<label>Seizoen<input required name="season" value="2026/27"></label><label>Eindstand Ajax<input min="0" type="number" name="ajaxScore"></label><label>Eindstand tegenstander<input min="0" type="number" name="opponentScore"></label></div><h2>Selecteer spelers</h2>${playerPicker()}<div class="sticky-action admin-actions"><button class="button secondary" name="intent" value="draft">Opslaan als concept</button><button class="button" name="intent" value="open">Stemming direct openen</button></div></form></main>`,"/motm-admin.js");
    if(id){
      ...
      return page("Stemming beheren",`<main class="motm-main admin"><a class="back" href="/club/motm/beheer">← Beheer</a><section class="match-head"><span class="status status-${match.status}">${statusLabel(match.status)}</span>${matchHeading(match)}</section><section class="manage-panel"><p><strong>${voteLabel(Number(count))}</strong></p><label>Deelbare URL<input id="share-url" readonly value="${esc(new URL(`/club/stemmen/${match.slug}`,request.url).href)}"></label><div class="share-actions"><button class="button secondary" type="button" data-copy>Kopieer link</button><button class="button secondary" type="button" data-share>Delen</button></div>${timing}${closed}</section></main>`,"/motm-admin.js");
    }
