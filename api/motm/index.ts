...
const matchRow=(match:any,ownVote?:string)=>`<a class="match-row" href="/club/stemmen/${esc(match.slug)}"><span class="status status-${match.status==="draft"?"planned":esc(match.status)}">${match.status==="draft"?"Gepland":esc(statusLabel(match.status))}</span><span class="match-row__copy"><strong>Ajax — ${esc(match.opponent)}</strong><small>${esc(formatKickoff(match.kickoff_at))}${ownVote?` · Jouw stem: ${esc(ownVote)}`:""}</small></span><b>Bekijk →</b></a>`;
...
export async function GET(request:Request){
  ...
  try{
    ...
    const section=(title:string,items:any[],note="")=>items.length?`<section class="match-section"><div class="section-heading"><div><p class="eyebrow">${note}</p><h2>${title}</h2></div></div><div class="match-list">${items.map((m:any)=>matchRow(m,m.own_vote)).join("")}</div></section>`:"";
